...
const ClubsChart = () => {
  return (
    <Box className={ClubsChartStyles.clubs}>
      <Typography className={ClubsChartStyles.clubsChartHeading}>Victorious Club/Team in Matches</Typography>
            ...
            <TableBody>
              {clubData.map((row) => (
                <TableRow key={row.id}  className={ClubsChartStyles.tableBody}>
                  <TableCell className={ClubsChartStyles.teamContainer} component="th" scope="row">
                  <Box className={ClubsChartStyles.teamColor} style={{ backgroundColor: row.color }}></Box>
                    <Box>{row.Teams}</Box>
                  </TableCell>
